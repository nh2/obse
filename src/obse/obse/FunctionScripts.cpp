...
public:
	...
	virtual Script * ReadScript() {
		...
		if (scrToken) {
			m_funcScript = OBLIVION_CAST(scrToken->GetTESForm(), TESForm, Script);
			delete scrToken;
			scrToken = NULL;
		}
			...
			if (ret) {
				switch (ret->Type()) {
					...
					case kTokenType_Array:
						*result = (OBSEArrayVarInterface::Array*)ret->GetArray();
						break;
					case kTokenType_String:
						{
							*result = OBSEArrayVarInterface::Element(ret->GetString());
							break;
						}
					default:
